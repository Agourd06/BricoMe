...
<head>
    ...
    <title>BricolMe Reservations</title>
</head>
...
<body>
    ...
    @section('Reservation')
        <section class="min-screen w-full p-4">
            ...
            <div id="error-message"
                class="bg-red-500  fixed right-20 rounded-md top-50 z-50 text-white p-4 text-center animate-bounce mb-4">
                {{ session('error') }}
            </div>
                    ...
                    <div>
                        @if ($differenceInHours > 10)
                            <div class="flex justify-center">
                                ...
                                @csrf
                                <input type="hidden" name="R_id" value="{{$reservation->id}}">
                        ...
                        @elseif ($differenceInHours >= 0)
                            <div class="flex flex-col items-center bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative">
                                <div>
                                    <h1 class="font-bold mb-2">It Was Good ?</h1>
                                </div>
                                <div class="flex items-center">
                                    <button onclick="toggleModal('rating')" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2">
                                        Review
                                    </button>
                                    <form action="/cancelReservation" method="POST" class="bg-black hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">
                                        @csrf
                                        <input type="hidden" name="R_id" value="{{$reservation->id}}" >
                                        <button type="submit">Delete</button>
                                    </form>
                                    <form action="/reporting" method="get" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mr-2">
                                        @csrf
                                        <input type="hidden" name="artisan_id" value="{{$reservation->artisan_id}}">
                                        <button type="submit">Repport</button>
                                    </form>
                                </div>
                            </div>
                        @endif
    ...
    @endsection
    <div id="rating" class="fixed z-50 hidden min-h-screen top-0 right-0 bottom-0 left-0 bg-gray-200/50">
        <div
            class="rounded-2xl flex flex-col mx-auto items-center justify-center bg-white shadow-xl mt-56 mx-8 mb-4 max-w-lg">
            ...
            <form id="ratingForm" action="/Rated" class="w-full max-w-xl bg-white rounded-3xl px-4 pt-2"
                onsubmit="submitRating()">

                <input type="hidden" name="rating" id="selectedRating" value="0">
                <!-- Hidden input field for rating value -->
...
<script>
    ...
    for (let i = 0; i < svgs.length; i++) {
        svgs[i].onclick = () => {
            ...
            document.getElementById('selectedRating').value = i + 1;
        }
    }
    ...
    function submitRating() {
        ...
        const client_id_input = document.createElement('input');
        client_id_input.type = 'hidden';
        client_id_input.name = 'client_id';
        client_id_input.value = {!! json_encode($reservation->client_id) !!};
        form.appendChild(client_id_input);

        const artisan_id_input = document.createElement('input');
        artisan_id_input.type = 'hidden';
        artisan_id_input.name = 'artisan_id';
